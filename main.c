...
uint32_t* AES_Encryption_Decryption(uint32_t input[4], uint32_t key[4],uint8_t type)
{
    ...
    uint32_t *state = addRoundKey(input,first_key); // {a0 , a1 , a2, a3}
    for(int i=0;i<9;i++)
    {
		uint32_t col[4] = {0};
        for(int j=0;j<4;j++)
...
uint32_t Rcon(uint32_t round_no)
{
    // round_no is 1_index not 0_index
    uint32_t output = RC[round_no - 1];
    output = output << 24;
    return output;
}
...
uint32_t* AES_Encryption(uint32_t input[4], uint32_t key[4],uint8_t type)
{
    uint32_t *w = KeyExpansion(key);
    uint32_t key_order ={0};

    uint32_t *state = addRoundKey(input,key); // {a0 , a1 , a2, a3}
...
int main()
{
    ...
    uint32_t ss[4] = {0x01234567,0x89abcdef,0xfedcba98,0x76543210};
    uint32_t *output = AES_Encryption(ss,key,'e');
    printf("Final z3bola : ");
    for(int i=0;i<4;i++){
        printf("%u ",output[i]);
    }
...
/*
void AES_Encryption(uint32_t input[4], uint32_t key[4],uint8_t type)
{
    uint32_t *input_of_round = addRoundKey(input,key);

    for(int i=0;i<10;i++)
    {
        uint32_t output_of_round[4] = {0};

        for(int j=0;j<4;j++)
        {
            uint8_t index_of_te_0 = (input_of_round[j]>>0);
            uint8_t index_of_te_1 = (input_of_round[j]>>8);
            uint8_t index_of_te_2 = (input_of_round[j]>>16);
            uint8_t index_of_te_3 = (input_of_round[j]>>24);
            output_of_round[j] = te0[index_of_te_0] ^ te1[index_of_te_1] ^ te2[index_of_te_2] ^ te3[index_of_te_3] ^ key[j];
            printf("output_of_round[0][%i] = %u  \n",j,output_of_round[j]);
        }
    }
}
*/
