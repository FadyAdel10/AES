...
uint32_t* invMixColumns(uint32_t w[44]){
    ...
    for(int i=4;i<40;i=i+4){
        ...
        for(int k=i;k<i+4;k++){
            uint32_t res = 0;
            // printf("const : %X \n",mul);
            // printf("column to be multiplied : %x \n",w[k]);
            //0e 0b 0d 09
            for(int j=0;j<4;j++){
                uint32_t mulresult = 0;
                uint32_t val1 = mul>>(24-j*8)&0xff ;//0E
                uint32_t val2 = w[k]>>(24-j*8)&0xff; //47 //0100 0111
                uint32_t index = 0;
                uint32_t val1cpy = val1;//0E --> 0000 1110
                while(val1cpy!=0){//0000 0011
                    if(index==0){
                        if(val1cpy & 0x1){
                            mulresult = val2;
                        }
                        ...
                        val2 = (val2>>7 & 0x1)? (((val2<<1) & 0xff) ^ 0x1B): (val2<<1 & 0xff); //0001 1100 ^ 0000 1110
                        if(val1cpy & 0x1){
                            mulresult^=val2;
                        }
                    }
...
uint32_t* AES_Encryption_Decryption(uint32_t input[4], uint32_t key[4],uint8_t type)
{
    ...
    // }
    // printf("\n");
    //    printf("After AddRoundKey (initial state):\n");
    // for (int i = 0; i < 4; i++) {
    //     printf("state[%d] = %#x\n", i, state[i]);
    // }
...
int main()
{
    uint32_t key1[4] = { 0x0f1471c9 , 0x47d9e859 , 0x0cb7add6 , 0xaf7f6798} ;
    uint32_t plaintext[4] = {0x01234567,0x85abcdef,0xfedcba98,0x76543210};
...
//key :0f1471c947d9e8590cb7add6af7f6798
//plaintext: 0123456785abcdeffedcba9876543210
