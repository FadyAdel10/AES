...
uint32_t* AES_Encryption(uint32_t input[4], uint32_t key[4],uint8_t type)
{
    ...
    for(int i=0;i<9;i++)
    {
        ...
        for(int j=0;j<4;j++)
        {
			if(type == 'e'){
				...
				col[j] = td0[(state[j] >> 24)] ^ td1[(state[(j+1)%4]>>16) & 0xff] ^ td2[(state[(j+2)%4]>>8) & 0xff] ^ td3[state[(j+3)%4] & 0xff] ^ w[((i+1)*4)+j];
			}
		}
    }
	...
	state = addRoundKey(type == 'e' ? shift_output : sub_output,final_key);
	return state;
}
...
int main()
{
    ...
    uint32_t *output1 = AES_Encryption(plaintext,key,'e');
    uint32_t *output2 = AES_Encryption(cipherText,key,'d');
	printf("Cipher : ");
	for(int i=0;i<4;i++){
		printf("%u ",output1[i]);
	}
